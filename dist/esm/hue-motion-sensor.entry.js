...
const MotionSensor = class {
    ...
    componentDidLoad() {
        ...
        _switch
            .pipe(filter(([v1, v2]) => v1 && !v2), tap(() => console.log("switch off")), mergeMap(() => from(fetch(`http://${this.raspberry}:8080/api/monitor/off`, {
            mode: 'no-cors'
        }))), takeUntil(this.onDestroy$))
            .subscribe();
        _switch
            .pipe(filter(([v1, v2]) => v2 && !v1), tap(() => console.log("switch on")), mergeMap(() => from(fetch(`http://${this.raspberry}:8080/api/monitor/on`, {
            mode: 'no-cors'
        }))), takeUntil(this.onDestroy$))
            .subscribe();
    }
